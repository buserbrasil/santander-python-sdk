...
class UnitTestSantanderApiClient(unittest.TestCase):
    ...
    def test_is_authenticated(self):
        ...
        assert self.client.is_authenticated is False, "Não tem token, deveria retornar False"

    @patch("santander_client.api_client.client.datetime")
    @patch.object(SantanderApiClient, "_request_token", return_value=get_dict_token_response())
    def test_authenticate(self, mock_request_token, mock_datetime):
        ...
        expected_expires_at = datetime.fromtimestamp(1000) + timedelta(seconds=expire_in)
        assert self.client.token_expires_at == expected_expires_at, "Expiração não bate"
        assert self.client.session.headers["Authorization"] == f"Bearer {access_token}", "O token deve bater"
        assert self.client.session.headers["X-Application-Key"] == "test_client_id", "O client_id deve bater"
        assert self.client.session.verify is True, "Deve estar configurado para verificar certificado"
        assert self.client.session.cert == "test_configure_session_cert_path", "O path deve bater"
    ...
    def test_ensure_requirements_calls_functions(self, mock_authenticate):
        self.client._ensure_requirements()
        mock_authenticate.assert_called_once(), "Deveria ter chamado authenticate pois não estava autenticado"
    ...
    def test_request_token(self, mock_post):
        ...
        self.assertEqual(token_data.get("access_token"), self.token_response_mock.get("access_token"))
        self.assertEqual(token_data.get("expires_in"), self.token_response_mock.get("expires_in"))
    ...
    def test_request(self, mock_ensure_requirements, mock_request):
        response_dict = get_dict_payment_pix_response("12345678", 299.99, "READY_TO_PAY", "12345678909", "CPF")
        request_dict = get_dict_payment_pix_request("12345678", 299.99, "12345678909", "CPF")
        ...
        assert response_data == response_dict, "Deveria ter retornado o dict de resposta"
        ...
        assert response_data == response_dict, "Deveria ter retornado o dict de resposta"
        mock_request.assert_called_once_with(
            "POST", f"{SANTANDER_URL}/test_endpoint", json=request_dict, params=None, timeout=60
        )

    def test_prepare_url(self):
        self.client.config.workspace_id = "d6c7b8a9e"
        assert self.client._prepare_url("test_endpoint/qr") == f"{SANTANDER_URL}/test_endpoint/qr"
        assert self.client._prepare_url("test/:WORKSPACEID") == f"{SANTANDER_URL}/test/d6c7b8a9e"
        assert self.client._prepare_url(":workspaceid/pix") == f"{SANTANDER_URL}/d6c7b8a9e/pix"
    ...
    def test_post_method(self, mock_request):
        ...
        mock_request.assert_called_once_with("POST", "test_endpoint", data={"post_data_key": "post_data_value"})

    @patch("santander_client.api_client.client.SantanderApiClient._request")
    def test_put_method(self, mock_request):
        self.client.put("test_endpoint", data={"put_data_key": "put_data_value"})
        mock_request.assert_called_once_with("PUT", "test_endpoint", data={"put_data_key": "put_data_value"})
    ...
    def test_patch_method(self, mock_request):
        ...
        mock_request.assert_called_once_with("PATCH", "test_endpoint", data={"patch_data_key": "patch_data_value"})
