...
def test_create_payment(payment_flow, api_client):
    ...
    response = {"id": "12345", "status": "PENDING_VALIDATION"}
...
def test_ensure_ready_to_pay(payment_flow, api_client):
    confirm_data = {"id": "12345", "status": "PENDING_VALIDATION"}
    api_client.get.return_value = {"id": "12345", "status": "READY_TO_PAY"}
...
def test_confirm_payment(payment_flow, api_client):
    ...
    payment_id = "12345"
    response = {"id": payment_id, "status": "PAYED"}
...
def test_check_for_rejected_error(payment_flow):
    response = {"status": "REJECTED", "rejectReason": "Insufficient funds"}
...
def test_request_payment_status(payment_flow, api_client):
    payment_id = "12345"
    response = {"id": payment_id, "status": "PENDING_VALIDATION"}
...
def test_payment_status_polling(
    ...
    lazy_status_update(
        payment_id,
        ["PENDING_VALIDATION"] * attemps_to_be_ready + ["READY_TO_PAY"],
    )

    result = payment_flow._payment_status_polling(payment_id, ["READY_TO_PAY"], 10)
    assert result.get("status") == "READY_TO_PAY"
...
def test_payment_status_polling_timeout(
    ...
    payment_id = "12345"
    lazy_status_update(payment_id, ["PENDING_VALIDATION"] * 3)

    with pytest.raises(SantanderStatusTimeoutError):
        payment_flow._payment_status_polling(payment_id, ["READY_TO_PAY"], 3)
    assert api_client.get.call_count == 3
